...
class DfiShipping extends ShippingAbstract
{
    ...
    /**
     * Alerts the configured notification_recipients (simpleshop.DfiShipping.Settings,
     * comma-separated) whenever the DFI shipping calculation fails and falls
     * back to the general_costs setting, so a broken API token/connection
     * doesn't go unnoticed.
     */
    protected static function notifyApiFailure(Order $order, DfiException $e, float $fallbackCost): void
    {
        $Settings   = \rex::getConfig('simpleshop.DfiShipping.Settings', []);
        $recipients = array_filter(array_map('trim', explode(',', (string) ($Settings['notification_recipients'] ?? ''))));
