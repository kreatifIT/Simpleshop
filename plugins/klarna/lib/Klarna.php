...
class Klarna extends PaymentAbstract
{
	...
	public function createSession()
	{
		...
		$products   = $order->getValue('products');
		$promotions = $order->getValue('promotions');
		if ($promotions['clubDiscount'] && $promotions['clubDiscount']->getValue('free_shipping')) {
			//nix mit shipping
		} else {
			$order->setValue('shipping', Order::prepareData(Shipping::get('default_shipping')));
		}
		...
		$orderProducts = $order->getValue('products');
		$promotions    = $order->getValue('promotions');
		$shippingCosts = $order->getValue('shipping_costs');

		/** @var Product $orderProduct */
		foreach ($orderProducts as $orderProduct) {
			$tax          = Tax::get($orderProduct->getValue('tax'));
			$quantity     = $orderProduct->getValue('cart_quantity');
			$orderLines[] = [
				'type'             => 'physical',
				'name'             => $orderProduct->getName() . ' - ' . $orderProduct->getValue('subtitle', true),
				'quantity'         => $quantity,
				'unit_price'       => number_format($orderProduct->getPrice(true), 2, '', ''),
				'tax_rate'         => number_format($tax->getValue('tax'), 2, '', ''),
				'total_amount'     => number_format($orderProduct->getPrice(true) * $quantity, 2, '', ''),
				'total_tax_amount' => number_format($orderProduct->getTax() * $quantity, 2, '', ''),
			];
		}

		foreach ($promotions as $promotion) {
			$discount    = $promotion->getValue('value') * -1;
			$taxDiscount = $promotion->getValue('tax_value') ? array_sum($promotion->getValue('tax_value')) * -1 : 0;
			$percentAge  = 22;
	...
	public function placeOrder($Order)
	{
		...
		if ($alias == '' || $secret == '') {
			throw new KlarnaException('The Klarna Credentials are not set!', 1);
		} elseif (!isset($authResponses['authorization_token'])) {
	...
	private function getOrderLines($order)
	{
		...
		foreach ($orderProducts as $orderProduct) {
			...
			$orderLines[] = [
				'type'             => 'physical',
				'name'             => $product->getName() . ' - ' . $product->getValue('subtitle', true),
				'quantity'         => $quantity,
