...
<div class="quantity-ctrl-button">
    <?php if ($config['has_quantity_control']): ?>
        <div class="amount-increment clearfix">
            <button class="amount-increment-minus button secondary float-left" type="button"
                    onclick="Simpleshop.changeCartAmount(this, '<?= $config['has_add_to_cart_button'] ? '' : $product_key ?>', <?= (int) $max_amount ?>);">-
            </button>
            <input type="text" class="amount-input float-left" value="<?= $quantity ?>" name="quantity[<?= $product_key ?>]"
                   onblur="Simpleshop.changeCartAmount(this, '<?= $config['has_add_to_cart_button'] ? '' : $product_key ?>', <?= (int) $max_amount ?>);">
            <button class="amount-increment-plus button secondary float-left" type="button"
                    onclick="Simpleshop.changeCartAmount(this, '<?= $config['has_add_to_cart_button'] ? '' : $product_key ?>', <?= (int) $max_amount ?>);">+
            </button>
        </div>
