...
$auth_config = FragmentConfig::getValue('auth');
...
?>
<div id="<?= $sid ?>" class="section-wrapper auth-wrapper <?= $auth_config['css_class']['wrapper'] ?>" data-auth-wrapper>
    <div class="login-form <?= $action == 'recover' ? 'hide' : '' ?>">
        <div class="grid-x grid-margin-x align-center">

            <?php if ($auth_config['has_registration']): ?>
                <?php
                ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                // REGISTRATION
                ...
                $customer_config = FragmentConfig::getValue('yform_fields.rex_shop_customer');
                ...
                foreach ($fields as $index => $field) {
                    ...
                    if ($field->getElement('type_id') == 'value') {
                        $params = array_merge($field->toArray(), (array)$customer_config[$field->getElement('name')], [
                            'notice' => null,
                        ]);
                ...
                // Submit
                $form->setValueField('html', ['', '<div class="margin-small-top">']);
                $form->setValueField('submit', [
                    'name'        => 'submit',
                    'no_db'       => 'no_db',
                    'labels'      => strtoupper(\Wildcard::get('action.register')),
                    'css_classes' => 'button expanded ' . $customer_config['css_class']['buttons'],
                ]);
                ...
                ?>
                <div class="large-4 medium-6 cell margin-large-bottom">
            ...
            <div class="large-4 medium-6 cell margin-large-bottom">
                <form action="<?= rex_getUrl(null, null, array_merge($_GET, ['ts' => time()])) ?>#-<?= $sid ?>" method="post">
                    <div class="login-panel">
                        ...
                        <button type="submit" class="button <?= $auth_config['css_class']['buttons'] ?>" tabindex="105" name="action"
                                value="login"><?= ucfirst(\Sprog\Wildcard::get('action.login')) ?></button>

                        <?php if ($auth_config['has_password_recovery']): ?>
                    ...
                    <div class="login-panel">
                        ...
                        <?php else: ?>
                            <form action="#-<?= $sid ?>" method="post">
                                ...
                                <button type="submit" class="button <?= $customer_config['css_class']['buttons'] ?>"
                        ...
                        <div class="margin-small-top">
                            ...
                            <br>
                            <?php if ($customer_config['has_registration']): ?>
                                ###label.no_login_data###: <a href="javascript:;" onclick="Simpleshop.toggleAuth(this, '.register-form')"
                                                              class="text-link">###label.register_now###</a>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
